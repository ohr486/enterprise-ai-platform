...
- **関連ドキュメント**: なし（本書はスタンドアロン）。実装プラン・横断サブシステム要件（MCP Gateway / Knowledge DB）・ADR は本書承認後に別途起票する
...
本書は要件レベル（What / Why / 制約 / 受け入れ基準）に閉じた **スタンドアロン要件定義書**である。実装プラン（How / フェーズ分割 / コード / Terraform スニペット）・横断サブシステム要件（MCP Gateway / Knowledge DB）・ADR は本書承認後に別途起票する。
...
> 本書承認後に起票する候補ドキュメント（現時点では未作成）:
> - 実装プラン（フェーズ分割・Terraform スニペット）
> - MCP Gateway 要件・プラン（横断サブシステム）
> - Knowledge DB 要件・プラン（横断サブシステム）
> - ADR: バックエンド言語統一、コンピュート分離戦略、監査長期保管、認証 IdP 段階導入 など
...
### 1.4 横断サブシステムとの関係（スコープ外）

将来的に以下の横断サブシステムを別途要件定義する想定だが、**本書のスコープには含まない**:

- **MCP Gateway**（上流 MCP サーバーの集約・名前空間化・ACL）
- **Knowledge DB (KDB)**（汎用 RAG、OpenClaw ランタイム検索 + ナレッジツール）

これらは別書で要件定義され、本体と VPC / Cognito / Admin Console / 監査基盤を共有しつつ、ECS タスク・DynamoDB テーブルは物理分離する想定。本書は **本体の固有要件のみ**を扱う。
...
| FR-AUD-06 | 監査書込抽象（リポジトリパターン）でテーブル名を注入可能とし、将来の横断サブシステム（MCP Gateway / KDB）から同一スキーマで書き込めるよう設計 | 決定 |
...
| 月次合計（本体 + KDB + MCP Gateway、観測性込み MCP $85 / $235） | $420〜$650 | $2,625〜$4,165 |
| 成功基準目標（500 × $5） | $250 | **$2,500** |

> 500 ユーザー時の最大想定（$4,165）は目標 $2,500 を超過しうる。Bedrock 推論コスト（最大費目、$800〜$1,500）の最適化と KDB / MCP Gateway の MVP 厳格化（既に実施）で吸収する方針。詳細は § 9.3。
...
**決定: B**。バックエンドサービスは Python 3.12 に統一する。本書承認後に ADR として正式起票する。
...
> 要件レベルで主要属性のみ列挙する。詳細スキーマ・GSI 設計・アクセスパターン仕様は本書承認後に別途実装プランで定義する。
...
GSI 設計（アクセスパターン別の検索キー）は本書承認後に別途実装プランで定義する。
...
| Bedrock Knowledge Bases | 将来導入する横断 KDB サブシステム経由のみ。本体エージェントは直接呼び出さない | 本書スコープ外 |
...
> 実装フェーズ分割（10 フェーズ）は本書承認後の実装プランで定義する。本節はマイルストーン粒度のみを示す。
...
| **目標（$5 / ユーザー / 月、本体のみ）** | $250 | **$2,500** |

> 横断サブシステム（MCP Gateway / KDB）の予算は別書で算定する。本書は本体のみを対象とし、500 ユーザー時の本体上振れ（$3,290）が目標 $2,500 を超える可能性がある。Bedrock 推論コスト（最大費目、$800〜$1,500）の最適化が一次的な吸収手段。
...
> 将来導入する横断サブシステム（MCP Gateway / KDB）の GDPR 削除権 SLA は本書 OQ-05 の確定方針に追従させる前提とする。

---

## 10. 受け入れ基準（要件達成の判定）

> **AC ID の独立性**: 本書の `AC-F-xx` / `AC-P-xx` / `AC-S-xx` / `AC-O-xx` は **本書内で完結する ID 体系**である。将来横断サブシステム（MCP Gateway / KDB）の要件定義書を起票する際、同名 ID が出現しうるため、横断参照する場合はファイル名込みで指定する（例: 本書 `AC-F-04` ≠ 将来の KDB `AC-F-04`）。
...
| AC-S-11 | **監査書込 IAM 境界テスト**: 本体タスクロールが本体監査テーブル以外（将来の MCP Gateway / KDB 監査テーブル含む）に書込できないことを (a) IAM Simulator API による宣言的検証 + (b) CI 環境での実 STS AssumeRole 試行による実アクセス検証 の **二段階** で確認。将来の横断サブシステムが追加された時点で対称テストを各書に追加する |
...
本セクションは Open Questions の確定履歴を集約する。本書の初版時点で、本体 OPENCLAW について以下の事項を **本書承認時点で確定済み**として扱う。一次出典は本書内の該当節および [`CLAUDE.md`](../CLAUDE.md) のスコープ制約宣言とする。

| 確定事項 ID | 確定日 | 決定者 | 決定内容 | 影響セクション |
|---|---|---|---|---|
| FIX-01 | 本書承認時 | プロダクトオーナー | ユーザー上限 500、単一リージョン、Slack のみ（CLAUDE.md スコープ制約と整合） | § 2.3, § 3.9, § 4.7 |
| FIX-02 | 本書承認時 | プロダクトオーナー | OpenClaw `2026.3.24` 固定、フォーク・パッチ禁止（CLAUDE.md「OpenClaw へのゼロ侵襲」と整合） | § 3.6 FR-AC-01, FR-AC-09 |
| FIX-03 | 本書承認時 | アーキテクト | バックエンドサービスを Python 3.12 に統一（§ 5.4） | § 5.4 |
| FIX-04 | 本書承認時 | アーキテクト | ランタイムは AgentCore（既定）+ ECS Fargate（Always-On）のハイブリッド（§ 5.1） | § 3.7, § 5.1 |
| FIX-05 | 本書承認時 | アーキテクト | 監査一次系（Firehose → S3 Object Lock → Athena）を MVP から構築（§ 3.14 FR-AUD-02） | § 3.14 FR-AUD-02 |
| FIX-06 | 本書承認時 | セキュリティ | L1 SOUL は Trust Boundary ではない（L3 以降のみ保証範囲、§ 4.4） | § 4.4 |
| FIX-07 | 本書承認時 | アーキテクト | Service Discovery は AWS Cloud Map（DNS TTL 10 秒）（§ 3.7 FR-RT-04） | § 3.7 FR-RT-04 |

> 上記 FIX-01〜FIX-07 は実装プラン未起票時点での仮確定。実装プラン起票時に各 FIX を一次出典としてプラン側にミラーする。
