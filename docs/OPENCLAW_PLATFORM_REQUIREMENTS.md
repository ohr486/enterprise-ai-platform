...
| KDB（別予算、KDB § 9.3 と整合） | $80〜$130 | $300〜$640 |
| MCP Gateway（別予算、MCP § 9.3 と整合、観測性込み） | $85 | $235 |
| **合計** | **$420〜$650** | **$2,625〜$4,165** |
| **目標（$5 / ユーザー / 月）** | $250 | **$2,500** |

> 500 ユーザー時の合計（$2,625〜$4,165）が目標 $2,500 を超過する可能性がある。Bedrock 推論コスト（最大費目、$800〜$1,500）の最適化と KDB / MCP Gateway の MVP 厳格化（既に実施）で吸収する。MCP は単一値（観測性込み $85 / $235）で計上し、本体・KDB・MCP の 3 書合算は本書 § 9.3 を一次出典とする。
...
##### 3 書の現状ステータス（OQ-05 確定までの仮置き）

| 書 | 該当要件 ID | 現状 | 備考 |
|---|---|---|---|
| 本体（本書） | NFR-CMP-03 | **未決定（OQ-05）** | 本書の確定で残る 5 件中の 1 つ |
| MCP Gateway | § 4.7 NFR-CMP-05 | **未決定（OQ-04）** | 「本体と整合（30 日想定）」と仮置き |
| KDB | § 4.7 NFR-CMP-04 + FR-VER-04 + AC-F-07 | **決定（30 日以内）** | KDB は単独で 30 日 SLA を採用済み。本書 OQ-05 確定までは「暫定」として読み替える |

##### 結論方針

- **GDPR 削除権 SLA の一次オーナーは本書 OQ-05**。3 書（本体 OQ-05 / MCP OQ-04 / KDB の既存記述）は同論点を別 ID で管理しているが、本書 § 9.4.1 を集約管理点とし、MCP OQ-04 / KDB は本書確定方針に追従する。
- **OQ-05 確定までは KDB の 30 日 SLA を 3 書共通の暫定値**として運用する（KDB 既存記述を尊重しつつ、本体・MCP は「未決定」のまま並走）。
- OQ-05 が「30 日」で確定した場合: 本体 NFR-CMP-03 / MCP NFR-CMP-05 を「決定」に昇格、KDB はそのまま維持。
- OQ-05 が「即時」または「30 日以外」で確定した場合: KDB の NFR-CMP-04 / FR-VER-04 / AC-F-07 を一斉に上書きする。
- いずれの場合も上記 3 案（擬名化 / 期間短縮 / データ分離）から実装方針を選定すること。

---

## 10. 受け入れ基準（要件達成の判定）

> **AC ID の独立性**: 本書の `AC-F-xx` / `AC-P-xx` / `AC-S-xx` / `AC-O-xx` は **本書内で完結する ID 体系**であり、MCP Gateway / KDB の同名 ID とは独立。横断参照する場合はファイル名込みで指定する（例: 本書 `AC-F-04` ≠ KDB `AC-F-04`）。書間で同一テストを指す場合のみ本文中で明示的にクロスリンクする（例: AC-S-11 ↔ MCP AC-S-06 ↔ KDB AC-S-06）。
...
| AC-S-11 | **AuditRepository IAM 境界テスト（3 系統）**: 本体 / MCP / KDB タスクロールの越境アクセスを二段階検証で全件拒否。実装プラン Phase 10 Step 3b の自然延長として CI 必須。詳細は下記注記 |

> **AC-S-11 詳細**: (a) IAM Simulator API による宣言的検証 + (b) CI 環境での実 STS AssumeRole 試行による実アクセス検証の **二段階** を実行する。検証範囲は 3 系統 × 越境先 2 系統 = 6 ケース（本体→MCP/KDB、MCP→本体/KDB、KDB→本体/MCP）を網羅。MCP § 10.3 AC-S-06 / KDB § 10.3 AC-S-06 は本要件を参照する 3 書共通テストスイートとして実装する。
