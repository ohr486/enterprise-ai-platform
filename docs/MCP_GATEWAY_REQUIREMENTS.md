...
| マイルストーン | 本体（§ 9.3 小計） | KDB（§ 9.3） | MCP Gateway（§ 9.3、観測性込み） | 合計 | 成功基準目標（$5/user × N） |
|---|---|---|---|---|---|
| MVP（M1: 50 ユーザー） | $255〜$435 | $80〜$130 | $85 | **$420〜$650** | $250 |
| 最終（M3: 500 ユーザー） | $2,090〜$3,290 | $300〜$640 | $235 | **$2,625〜$4,165** | **$2,500** |

> 500 ユーザー時の最大想定（$4,165）が目標 $2,500 を超過しうる点は本体要件書 § 9.3 と同認識。Bedrock 推論コストの最適化で吸収。MCP は § 9.3 の観測性込み単一値（$85 / $235）を採用し、§ 4.9 単独表の $80 / $220 は観測性除外の参考値として残す。
...
> 本体 $2,090〜$3,290 + KDB $300〜$640 + MCP Gateway $235（観測性込み）= **500 ユーザー時合計 $2,625〜$4,165 / 月**（本体要件書 § 9.3 と同値）。本体「1 ユーザー / 月 $5 以下」目標（500 ユーザーで $2,500）は最大想定で超過するため、Bedrock 推論コストの最適化で吸収する方針（本体要件書 § 9.3 と同認識）。
...
> **AC ID の独立性**: 本書の `AC-F-xx` / `AC-P-xx` / `AC-S-xx` / `AC-O-xx` は **本書内で完結する ID 体系**であり、本体 OPENCLAW / KDB の同名 ID とは独立。横断参照する場合はファイル名込みで指定する（例: 本書 `AC-F-04`「名前空間衝突 Fail Fast」≠ 本体 `AC-F-04`「Auto-Provisioning」）。書間で同一テストを指す場合のみ本文中で明示的にクロスリンクする（例: 本書 AC-S-06 ↔ 本体 AC-S-11）。
...
| AC-S-06 | **AuditRepository IAM 境界テスト（3 系統）**: Gateway タスクロールが本体 / KDB テーブルへ越境アクセスできないことを二段階検証で全件拒否（本体 § 10.3 **AC-S-11** と同方針、3 書共通テストスイートとして実装）。検証手段の詳細（IAM Simulator + 実 STS AssumeRole）は本体 AC-S-11 注記を参照 |
...
| OQ-04 | GDPR 削除権 SLA（30 日 vs 即時）と監査ログ保持要件の整合。**本体 OQ-05 と同論点**（一次オーナーは本体）— 本体要件書 § 9.4.1 で 3 書ステータスと結論方針を集約管理。本書は本体確定後に追従 | コンプライアンス | 本体 OQ-05 確定に追従 |
